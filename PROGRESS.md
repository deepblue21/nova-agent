...
- **SIRADAKİ (kullanıcı, Windows/WSL):** `bash scripts/clean-history.sh` (veya `clean-history.ps1`, ya da fresh-repo: `rm -rf .git && git init`), sonra ya `git push --force origin main` ya da GitHub repo'yu silip yeniden oluşturup push. da8bece GitHub cache/fork/PR'da kalabileceği için tam temiz için repo'yu silip-yeniden-oluşturmak en güvenlisi. Push sonrası `da8bece` commit URL'inin 404 verdiğini doğrula.
