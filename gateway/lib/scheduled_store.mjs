...
const COLS =
  "id, title, prompt, model, agent, schedule, enabled, " +
...
export async function listTasks(userId) {
  const r = await q(`SELECT ${COLS} FROM scheduled_tasks WHERE user_id=$1 ORDER BY created_at DESC`, [userId]);
  return r.rows;
}

export async function createTask(userId, { title, prompt, model, agent, schedule, nextRunAt }) {
  const r = await q(
    `INSERT INTO scheduled_tasks (user_id, title, prompt, model, agent, schedule, next_run_at)
     VALUES ($1,$2,$3,$4,$5,$6, to_timestamp($7/1000.0))
     RETURNING ${COLS}`,
    [userId, title, prompt, model || null, !!agent, schedule, nextRunAt]);
  return r.rows[0];
}
